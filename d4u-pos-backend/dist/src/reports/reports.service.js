...
let ReportsService = class ReportsService {
    ...
    async getDailyReport(store_id, date) {
    ...
    async getBranchAnalytics(store_id, start_date, end_date, business_day_id, cashier_id) {
        const where = {
            store_id,
            status: { not: 'VOIDED' },
        };
        ...
        const [posOrders, onlineOrdersDb, voidedCount] = await Promise.all([
            this.prisma.order.findMany({
                where: { ...where, is_offline: false, order_source: { in: ['WALKIN', 'DINEIN'] } },
                include: { items: true, approver: { select: { name: true } } },
            }),
            this.prisma.order.findMany({
                where: { ...where, order_source: { in: ['ONLINE', 'WHATSAPP', 'CALL'] } },
                include: { items: true },
            }),
            this.prisma.order.count({
    ...
    async getShifts(store_id, limit = 10) {
    ...
    async getTopProducts(store_id, limit = 10) {
    ...
    async getVoidedOrders(store_id, business_day_id) {
        const where = { store_id, status: 'VOIDED' };
        if (business_day_id)
            where.business_day_id = business_day_id;
    ...
    async getBrandOverview(brand_id) {
        const stores = await this.prisma.store.findMany({ where: { brand_id } });
        const overview = await Promise.all(stores.map(async (store) => {
